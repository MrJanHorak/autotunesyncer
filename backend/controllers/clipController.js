...
export const saveClip = (req, res) => {
  ...
  const existing = db
    .prepare(
      'SELECT file_path FROM project_clips WHERE project_id = ? AND instrument_key = ?',
    )
    .get(projectId, key);
...
export const listClips = (req, res) => {
  ...
  const clips = db
    .prepare(
      'SELECT instrument_key FROM project_clips WHERE project_id = ? ORDER BY created_at',
    )
    .all(projectId);
  res.json({ clips });
};
...
export const deleteClip = (req, res) => {
  ...
  if (clip?.file_path && existsSync(clip.file_path)) {
    try {
      unlinkSync(clip.file_path);
    } catch {
      /* ignore */
    }
  }
  db.prepare(
    'DELETE FROM project_clips WHERE project_id = ? AND instrument_key = ?',
