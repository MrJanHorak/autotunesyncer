...
class EnhancedVideoProcessor:
    ...
    def _generate_combination_filter(self, input_files: List[str], midi_data: Dict) -> str:
        ...
        if num_videos == 2:
            ...
            # Dynamic grid layout
            cols = min(3, int(num_videos ** 0.5) + 1)
            rows = (num_videos + cols - 1) // cols
            
            # Build complex filter for dynamic grid
            filter_parts = []
            for i in range(num_videos):
                filter_parts.append(f'[{i}:v]scale=320:240[v{i}]')
            
            # Add grid composition logic here based on requirements
            audio_mix = '+'.join([f'[{i}:a]' for i in range(num_videos)])
            filter_parts.append(f'{audio_mix}amix=inputs={num_videos}[outa]')
